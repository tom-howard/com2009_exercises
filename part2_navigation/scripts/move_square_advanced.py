...
class Square():
    ...
    def __init__(self):
        node_name = "square_dance_advanced"
    ...
    def count_squares(self, max_squares = 2):
        self.squares += 1/4
        print(f"{self.squares:.2f}/{max_squares:.0f} square(s) complete...")
        if self.squares >= max_squares:
            print("Job done, stopping.")
            self.ctrl_c = True
        ...
        while not self.ctrl_c:
            if self.turn:
                ...
                if displacement >= 1:
                    ...
                    displacement = 0.0
                    self.theta_z0 = self.theta_z
                    self.count_squares(max_squares=2)
                else:
                    # Not there yet, keep going:
